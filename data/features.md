...
本文统一描述 forecast 与 control 两个任务所需的全部特征。两个任务共用同一分钟决策网格（design 2.2）：连续竞价压缩为 237 个分钟格，每分钟末快照为该分钟的锚点 tick，缓存按分钟锚点存一行——行 $m$ 的特征由锚点前 $L=30$ 分钟内的全部快照构成（tick 数随快照密度变长）。forecast 在空仓期间每分钟锚点判一次门控，预测随后 $H=15$ 分钟的路径统计量；control 在固定每 10 分钟的锚点决策（design 4.1），决策落在哪个分钟就取哪一行。
...
只保留连续竞价时段（09:30–11:30 含 11:30、13:00–14:57 不含 14:57）且 `LastPx`、`Buy1Price`、`Sell1Price` 均为正的快照（剔除集合竞价、午休与盘口单边为空的快照）；同一标的、日期和 `MDTime` 重复时，保留排序后的最后一条。缓存的行索引即压缩分钟索引（0..236），锚点 tick 为该分钟的末快照（`anchor_ticks` 数组，无快照的分钟记 $-1$）：

- 行 $m$ 的输入窗口为分钟 $[m-L+1,\,m]$ 内的全部快照（右端收于锚点），仅 $m\ge L-1$、该分钟有锚点且窗口内中间价路径与累计成交字段均有效时计算特征；
- 目标路径以锚点为起点、连至分钟 $m+H$ 锚点（缺失时前向填充到最近锚点）的全部快照，仅 $m\ge L-1$（与特征行对齐）、$m+H\le 236$ 且前瞻路径上同样字段均有限时有目标。

窗口不完整、前瞻越界与无锚点的行一律记 $\mathrm{NaN}$；缓存不填零、不增加缺失指示列，LightGBM 原生处理缺失值（control 读取时按 §5.2 口径置零），无锚点分钟由使用方按锚点前向填充。

forecast 的训练、评估与推理行即全部有效锚点行；5 个目标全部有限且至少 1 个输入特征有限的行才进入训练集。
...
公共特征是经过窗口计算后 forecast 与 control 共同使用的特征：47 维窗口统计（§3.1–§3.6，每分钟锚点一行）是 forecast 的模型输入，其中不与微观 / 私有序列重复的 24 维同时进入 control 的宏观向量（§5.2）；标的标识 `symbol_id`（§3.7）在两侧分别作为分类输入与 embedding 索引。
...
| `kyle` | $\dfrac{\sum_{i=1}^{n}r_i x_i}{\sum_{i=1}^{n}x_i^2}$ | 价格冲击系数：单位有向成交量的价格冲击 |
...
这些状态只使用锚点 $t$ 及其之前的数据。当日首条快照的累计成交量记为 $V_{0}$，行末 $b$ 分钟内的快照构成当前 bar（$b$ 为 `bar_min`，上一 bar 边界为分钟 $m-b$ 的锚点 $t_b$），其成交量与当日已进行的平均每 bar 成交量为

$$
V_{\mathrm{bar}}=V_t-V_{t_b},\qquad \bar V_{\mathrm{bar}}=\frac{V_t-V_{0}}{M_{\mathrm{elapsed}}/b},
$$

其中 $M_{\mathrm{elapsed}}$ 为当日已进行的分钟数；bar 边界不存在（bar 之前当日无快照）时 `vol_rel` 与 `gap` 记 $\mathrm{NaN}$。
...
| `day_pos` | $t/(T-1)$（$T$ 为当日快照数） | 日内时间进度 |
| `session` | $\mathbf{1}_{\{t\text{ 处在下午时段}\}}$ | 上 / 下午时段标记 |
| `gap` | $p_{t_b+1}-p_{t_b}$ | 当前 bar 边界处的跳空 |
...
预测算法的输入为 §3 的公共特征全集：47 维窗口特征加 `symbol_id` 分类列，共 48 维。输出为 5 个前瞻目标的逐锚点预测。

### 4.1 预测目标

5 个目标均为 §3.2 的路径统计量在前瞻路径（自锚点起未来 $H=15$ 分钟的全部快照）上的取值：
...
强化学习算法（`control/`）的观测由微观序列、宏观向量与私有序列三部分组成，另附公共标识 `symbol_id`（§3.7）。微观与宏观特征逐标的只用训练集拟合 z-score（截断 $\pm10$），验证集和测试集复用同一统计量。

### 5.1 微观序列（$30\times66$）

回看窗口每分钟末快照（锚点）抽样的 30 步序列，末帧恰为决策点所在快照；无快照的分钟前向填充到最近锚点。前 40 维为十档相对价量：各档价格相对买一价取相对偏离，各档数量相对一档量取 $\log(1+\cdot)$。后 26 维为逐快照微观结构量——§3 的公共特征中凡为「逐快照量的窗口聚合」者，其逐快照原语均以同一定义纳入本序列：
...
$11+24+5$ 维：前 11 维由回看窗口聚合为 $L/b=30$ 根 $b$ 分钟 bar（中间价 OHLC 与分钟成交量，无快照的分钟按最近 close 填充、量能记 0），取当前 bar 的开 / 高 / 低相对其收盘价、收盘价环比、5/10/15/20/25/30-bar 均价偏离和相对成交量；中 24 维为 §3 公共特征的子集；末 5 维为 LightGBM 对 §4.1 五个前瞻目标的预测。后 29 维直接取自统一缓存 `cache/<symbol>.npz`。
...
窗口统计与预测直接取决策锚点所在分钟的行：该行的窗口恰好收于锚点，不含未完结数据。$m<L-1$ 时窗口不完整，该行与缺失、退化的窗口一律记零。这 29 维是整窗口的聚合量与预测值（相邻行重叠 $(L-1)/L$），不是逐时刻原语，故进宏观 MLP 而非 LSTM。

### 5.3 私有序列（$30\times11$）

与微观序列同一时间索引（分钟锚点）抽样，每步 11 维：前 7 维为账户与网格状态，后 4 维为决策区间内的成交过程通道。记底仓为 $Q_0$、初始权益为 $E_0$、当前网格中心为 $c$，半宽档与数量档的梯子上界为 $h_{\max}$ 和 $q_{\max}$：
...
中心偏离和当前网格参数使模型能够恢复区间内两条边界相对现价的位置；中心在每笔成交后重置为成交价、区间无成交则在决策锚点重置为当拍中间价（design 3.5）。

成交过程通道让状态包含决策区间内的买卖过程——买卖顺序与成交价相对区间起点的位置。成交笔数逐区间不定长，编码取时间网格聚合而非事件序列：私有序列本就是每分钟一步的时间网格，每步聚合该分钟内的成交即可。记 $p_{\mathrm{dec}}$ 为该步所属决策区间起点（决策锚点）的中间价（回看窗横跨约 3 个决策区间，前序区间的成交过程同在序列内，「区间起点」按各步所属区间取）：

| 维度 | 定义 | 补充信息 |
| --- | --- | --- |
| 步内买入笔数 | 该分钟买入成交笔数，$\log(1+\cdot)$ | 分侧后同分钟往返不被净额抵消 |
| 步内卖出笔数 | 同上，卖侧 | 买卖顺序由序列位置携带 |
| 最近成交价偏离 | 步末最近一笔成交价相对 $p_{\mathrm{dec}}$，/ATR；区间内无成交记 0、有成交沿用前值，新区间清零 | 成交价的绝对锚点 |
| 区间成交 VWAP 偏离 | $\sum_i s_iq_i(f_i-p_{\mathrm{dec}})\,/\sum_i q_i$ /ATR（区间内累计，$s$ 方向、$q$ 手数、$f$ 成交价；无成交记 0） | 配对价差直接原料：买卖配对时 $p_{\mathrm{dec}}$ 抵消、直接给出配对价差 |

现有 7 维已隐含的信息不重复：仓位通道逐步差分即每步净成交（方向 + 顺序）、中心偏离在成交后隐含现价相对最近成交价、距上次成交已有；缺的正是步内总笔数（净额掩盖一买一卖的往返，而往返恰是闭环事件）、成交价相对区间起点的绝对锚点与分侧价格。同一分钟内多笔成交的先后次序丢失（引擎每快照最多一笔，分钟内多笔少见），分侧计数足以标识往返，不为此引入事件级编码。
